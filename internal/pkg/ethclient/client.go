...
//go:generate mockgen -source=client.go -destination=mock/client_mocks.go -package ethclient_mock
...
func doRequest[T any](req *http.Request, client Doer) (result Response[T], err error) {
	...
	if err != nil {
		return result, fmt.Errorf("copy response body: %w", err)
	}

	var rpcResp Response[T]
	err = json.NewDecoder(teedReader).Decode(&rpcResp)
	if err != nil {
